...
function mapIdentityVerification(data) {
    return {
        id: Number(data.id),
...
function mapConditionVerification(data) {
    return {
        id: Number(data.id),
        booking_id: Number(data.booking_id),
...
let VerificationsService = class VerificationsService {
    ...
    async submitIdentityVerification(dto, user) {
        ...
        if (Number(booking.customer_id) !== user.profile.id) {
            throw new common_1.ForbiddenException('Tidak boleh mengirim verifikasi booking ini');
        }
        if (['rejected', 'cancelled', 'completed', 'expired'].includes(booking.status)) {
            throw new common_1.BadRequestException('Verifikasi identitas tidak bisa dikirim untuk status booking ini');
        }
    ...
    async submitConditionVerification(dto, user) {
        const booking = await this.prisma.booking.findUnique({
            where: {
                id: BigInt(dto.booking_id),
            },
        ...
        const itemIsPartOfBooking = booking.booking_items.some((bookingItem) => Number(bookingItem.item_id) === dto.item_id);
        ...
        if (dto.type === 'before_rent') {
            if (!['approved', 'ongoing', 'paid'].includes(booking.status)) {
                throw new common_1.BadRequestException('Foto kondisi sebelum sewa hanya bisa dikirim setelah booking disetujui');
            }
        }
        if (dto.type === 'after_rent') {
            if (!['ongoing', 'completed'].includes(booking.status)) {
                throw new common_1.BadRequestException('Foto kondisi setelah sewa hanya bisa dikirim saat booking berjalan atau selesai');
            }
        }
    ...
    async approveIdentityVerification(id, admin) {
        ...
        if (!verification) {
            throw new common_1.NotFoundException('Verifikasi identitas tidak ditemukan');
        }
    ...
    async rejectIdentityVerification(id, admin, dto) {
        const verification = await this.prisma.identityVerification.findUnique({
            where: {
                id: BigInt(id),
            },
    ...
    async approveConditionVerification(id, admin) {
        ...
        if (!verification) {
            throw new common_1.NotFoundException('Verifikasi kondisi barang tidak ditemukan');
        }
    ...
    async rejectConditionVerification(id, admin, dto) {
        const verification = await this.prisma.conditionVerification.findUnique({
            where: {
                id: BigInt(id),
            },
